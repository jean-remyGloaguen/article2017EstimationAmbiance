...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{bloc_diagram_source_separation.pdf}
...
Prior to data assimilation, the issue is to correctly estimate the traffic sound level from acoustic measurements as studied in \cite{leiba2017large,socoro_anomalous_2017}. As the urban sound environment is a complex environment gathering lots of different sounds (car passages, voices, whistling bird, car horn, airplanes\dots) that overlap, the estimation of the traffic sound level  based on acoustic measurements is not a trivial task.

\section{Related work}

Recent work is mostly focused on the detection or recognition tasks of environmental sounds \cite{heittola_sound_2011}, \cite{defreville_automatic_2006}, \cite{dufaux_automatic_2000}, \cite{chu_environmental_2009}. A two-step process is generally followed: describe the audio files with a set of features (Spectral Centroid, harmonicity, Mel-Frequency Cepstral Coefficient \dots) and classify them with the help of classifiers (Support Vector Machines, Gaussian Mixture Models, Hidden Markov Model, Neural Networks) \cite{cowling_comparison_2003} and can be applied to a variety of tasks \cite{shen_environmental_2012}, \cite{beritelli_pattern_2008}, \cite{couvreur_automatic_2004}.


The main issue with the detection or recognition tasks is the overlap of environmental sounds. Although near major roads, traffic is predominant, there are many places where it overlaps with other sound sources which contribute significantly to the overall sound levels. To circumvent this issue, Socor\'o et al. propose to suppress time frames where there is significant overlap by considering an Anomalous Noise Events Detector \cite{socoro_anomalous_2017}. It consists in detecting the unwanted sound sources from labeled recordings, \textit{i.e.} that are not related to the traffic component. Those time frames are then discarded in order not to take them into account during the estimation of the traffic sound level.
An alternative approach that we will follow in this paper is to consider the blind source separation paradigm to reliably estimate the traffic noise level, see Figure \ref{fig:diagram}. It consists in separating the contribution of the traffic from the other sources within a polyphonic scene. One major advantage of following such approach is that the estimate is  available continuously, making the approach applicable in a wide range of urban areas, even where the traffic noise is relatively low compared to the remaining contributions.\\

In an urban environment context, source separation can be achieved with the help of acoustic microphone arrays and beamforming \cite{saruwatari2003blind}. However, this approach requires the spreading of multiple microphones arrays in cities that is very expensive (even with low cost microphones) and time-consuming for calibration and maintenance.  Monophonic sensor networks need less microphones but the main challenge is to succeed to estimate correctly the level of the road traffic from a sound mixture in which all kind of sound sources can be present. A convenient method for this is the  Non-negative Matrix Factorization (NMF) technique \cite{lee_learning_1999}. When considering audio as input, it usually consists in approximating the magnitude spectrogram of an audio file by the product of two low rank matrices, one representing the components of interest and the other the contribution at a given time of those components to approximate the input magnitude spectrogram \cite{smaragdis_non-negative_2003} \cite{wilson_speech_2008}. In the audio processing domain, NMF has already been employed for the source separation task of monaural signals of speech and music \cite{wang_musical_2005} \cite{wilson_speech_2008}. By design, this method deals reasonably well with the overlapping sound sources as soon as the overlap can be mostly resolved on the time/frequency plane.
...
Here, we extend this approach with more flavors of NMF on more challenging sound scenes. The corpus of simulated sound scenes considered here is created from a built-up sound database composed of a large number of diverse sound samples in order to encompass a wide variety of sound environment with a variable presence of road traffic noise. The use of simulated sound scenes allows rigorous experimental validation as it offers a high level of control on the design of the scenes and the knowledge of the exact contribution of the traffic component ($L_{p,traffic}$), which would be difficult to extract from a recording of an urban scene.
The aim is to find the type of NMF that gives the minimal error on the estimation of the traffic noise signal level on the whole corpus. We demonstrate that supervised NMF and semi supervised NMF approaches have some interests but do not give satisfactory results for the application at hand. We thus introduce another scheme called thresholded initialized NMF that makes good use of prior knowledge about the source of interest, in our case the traffic noise, but also generalizes well to several kinds of urban areas and to traffic to interference ratio ($TIR$).
...
To better take into account prior knowledge about the sources of interest, constraints (like the smoothness or the sparsity criteria \cite{virtanen_monaural_2007}) can be added to the objective function.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\linewidth]{distanceCosLinDisplay.pdf}
\caption{$\mathbf{W}_{traffic}$ extraction from the sorted cosine similarity with a threshold $t = 0.6$. The first $82$ elements are here considered as traffic component.}
\label{fig:W_TI_NMF}
\end{figure}

This approach is named \textit{Thresholded inititialized NMF} (TI-NMF). Other thresholding methods as the \textit{soft} \cite{donoho1995noising} and the \textit{firm} \cite{fornasier2008iterative} have been investigated. A quick parametric study revealed that the \textit{hard} thresholding method, as presented in  Figure \ref{fig:W_TI_NMF}, was the most simple and reliable approach.
...
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{bloc_diagram_estimator.pdf}
...
Assuming that the traffic spectral profile is mostly concentrated in the low frequency components, a simple estimator to determine the traffic sound level is a frequency low-pass filter. It is parametrized only by its cut-off frequency $f_c \in  \lbrace$500, 1k, 2k, 5k, 10k, 20k$\rbrace$ Hz. The spectrogram $\mathbf{V}$ is filtered and the remaining energy is then considered as traffic component (eq. \ref{eq:v_tr_filtered}),

\begin{equation}\label{eq:v_tr_filtered}
\mathbf{\tilde{V}}_{traffic} = \mathbf{V}_{f_c}.
\end{equation}

The second estimator is the proposed scheme, based on one of the three NMF approaches presented in Section \ref{part:nmf}. Multiples experimental factors are involved here between the dictionary learning and NMF (see Figure \ref{fig:bloc_nmf}), each experimental factor having multiples modalities.

\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{bloc_diagram_NMF_EN_2.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{creation_W_EN.pdf}
...
\begin{figure}[t]
  \centering
  \subfloat[]{\label{fig:specW}\includegraphics[width=.45\linewidth]{dictionary3.pdf}}
%  \hspace{5pt}
  \subfloat[]{\label{fig:ElementW}\includegraphics[width=.45\linewidth]{dictionary4.pdf}}
...
100 iterations are performed for every NMF types. The spectrogram $\mathbf{V}$ and the dictionary $\mathbf{W}$ are expressed with third octave bands ($F$ = 29). This coarser method allows us to reduce the dimensionality and thus decrease the computation time. Furthermore, by expressing the frequency axis on a log frequency axis, the low frequencies, where the traffic energy is focused, are described more finely than the high frequencies. Experimental validation consistently showed that considering third octave bands do not impact the performance of the estimator studied in this paper. But, most of all, it is a suited representation to this sound environment as this kind of representation is widely used in the urban acoustic field, compared to MFCCs for instance. For TI-NMF, a preliminary study showed that the range of threshold values can be evaluated between 0.30 and 0.70. An increment step of 0.01 has been considered as being sufficiently precise. Table \ref{tab:experimental_factorsNMF} summarizes the experimental factors and their related modalities.

Considering the experimental settings derived from the different modalities of each experimental factor  between the 5 levels of $TIR$, the 6 sub-classes and the 6 cut-off frequencies $f_c$, 180 settings are performed (6 $\times$ 5 $\times$ 6). For Sup-NMF and Sem-NMF, according to Table \ref{tab:experimental_factorsNMF},  1200 combinations of factors are made where the 4 levels of $K$ are associated with $w_t \in$ $\lbrace$0.5, 1$\rbrace$ second whereas only 2 levels of $K$ (25 and 50) are associated with $w_t = all$, see part \ref{part:dictionary_learning} (6 $\times$ 5 $\times$ 2 $\times$ (2 $\times$ 4 + 1 $\times$ 2) $\times$ 2). For TI-NMF, because of the high number of threshold $t$ tested, 24600 combinations are computed (6 $\times$ 5 $\times$ (2 $\times$ 4 + 1 $\times$ 2) $\times$ 41). In all, 25980 settings are performed.
...
\midrule
t-test & Welch & Welch & Student \\
$t$-value & 7.81 & 2.84 & 4.60\\
Dof & 1478 & 1470 & 1498\\
$p$-value & \textbf{1.06 $\times 10^{-14}$} & \textbf{4.49 $\times 10^{-3}$} & \textbf{4.39 $\times 10^{-6}$} \\
\bottomrule
\end{tabular}
\caption{$p$-values deducted from the paired sample one-sided t-test of the SUP , SEM and TI NMF that offer the lowest $MAE$ error. In bold letters, the $p$-values that reject the $H_0$ hypothesis ($p$-value $\leq \alpha$).}
\label{tab:student_test}
\end{table}

For each couple of methods, the $H_0$ hypothesis is rejected meaning that the distributions of the sound level estimations according to the approaches are significantly different despite similar mean errors. In the case of the comparison of the distributions of the SEM NMF with the TI NMF, a Student's t-test is calculated. But since the standard deviations are different between the SUP NMF and SEM NMF distributions and between the SUP NMF and TI NMF distributions, a Welch's t-test is performed, which is an adaptation of the Student's t-test. Consequently, the calculation of the degree of freedom is then obtained as a function of these variances and differs from that obtained by the Student t-test. To better understand these differences, the errors made according to the $TIR$ and for each sound environment are displayed in Table \ref{tab:results_TIR} and in Figure \ref{fig:TIR_bar}.
...
\begin{figure*}[t]
  \centering
  \subfloat[$MAE$ error for each $TIR$ and sub-class with the frequency low-pass filter with $f_c$ = 500 Hz.]{\label{fig:TIR_class_filter}\includegraphics[width=0.45\textwidth]{filter_bar.pdf}}
  \hspace{5pt}
  \subfloat[$MAE$ error for each $TIR$ and sub-class with Sup-NMF and $\beta$ = 2.]{\label{fig:TIR_class_sup}\includegraphics[width=0.45\linewidth]{sup_bar.pdf}}
  \hspace{5pt}
  \subfloat[$MAE$ error for each $TIR$ and sub-class with Sem-NMF and $\beta$ = 1.]{\label{fig:TIR_class_semi}\includegraphics[width=.45\linewidth]{semi-sup_bar.pdf}}
  \hspace{5pt}
  \subfloat[$MAE$ error for each $TIR$ and sub-class with TI-NMF, $\beta$ = 1 and $t$ = 0.41.]{\label{fig:TIR_class_TI}\includegraphics[width=.45\linewidth]{TI_bar}}
...
With the addition of a mobile part in the dictionary, $\mathbf{W_r}$, the semi-supervised approach allows us to better take into account the interfering class in low $TIR$. It thus brings a significant decrease of the errors for low $TIR$. However, the relatively high degrees of freedom of Sem-NMF are restrictive for high $TIR$ as the errors exceed 2 dB for all sub-classes and increase with $TIR \in \lbrace$6, 12$\rbrace$ dB. The performances are even superior to the LP filter baseline. In order to reduce $D(\mathbf{V} \vert \vert \mathbf{WH}$), without constraint, Sem-NMF is free to include traffic components in $\mathbf{W_r}$. Consequently, this behavior decreases the quality of the reconstruction of the traffic component.

The TI-NMF behavior, with a threshold $t = 0.41$ and $\beta$ = 1, generates smaller errors compared to the baseline for all the $TIR$ values, with the exception at $TIR$ = 0 dB. If this approach gives the lowest error on all the corpus, it is not the one that propose on each $TIR$ the best performances: SUP-NMF has lower errors for positive $TIR$ while SEM-NMF surpasses it for $TIR \in \left\lbrace -12, -6\right\rbrace$. But the specific case $TIR = -12 dB$ is a less likely limit case in urban environment. Putting this specific case aside, TI-NMF is the only approach that propose an error inferior to 3 dB for $TIR \in \left\lbrace -6, 12 \right\rbrace$ dB. Unlike Sup-NMF, where $\mathbf{W}$ is fixed, and Sem-NMF, where only $\mathbf{W_r}$ is updated, TI-NMF updates $\mathbf{W}$ entirely to adjust prior knowledge to the scene under evaluation so as to adapt to the different sound environments. The closest elements of the traffic component defined in $\mathbf{W_0}$ are then extracted to deduce the traffic signal.
...
For high $TIR$, as the traffic is the main sound source, the similarity of the initial dictionary and $\mathbf{W}$ is higher which allows us to retain more elements as traffic components (181 basis for $TIR$ = 12 dB) and then decreases the error ($MAE$ < 1). The kept elements are then more suited to the scenes than a fixed dictionary. The error for these $TIR$ is then due to the thresholding which put aside some elements that can be related to the traffic component.

It would be possible to decrease the error by adapting the threshold according to the $TIR$. For instance at $TIR$ = 12 dB, the average error on all the sub-classes would decrease to 0.30 ($\pm$ 0.11) with $t$ = 0.34 where 191 elements are considered as traffic component. On contrary, at $TIR$ = -12 dB, by increasing the threshold to 0.53, the error would decrease to 4.49 ($\pm$ 1.75) with in average 73 elements considered as \textit{traffic} elements.

But, in order to generalize this method to a practical case where no prior knowledge on the urban environment is available, the chosen threshold $t$ is then fixed to $t$ = 0.41 as it is the one that best balance these opposite cases.
...
\begin{figure*}[t]
  \centering
  \subfloat[]{\label{fig:specLp}\includegraphics[width=.45\linewidth]{specAlert.pdf}}
%  \hspace{5pt}
  \subfloat[]{\label{fig:lp_alert}\includegraphics[width=.45\linewidth]{NMF_Lp_alert.pdf}}
...
The non negative matrix factorization framework has been considered to estimate the road traffic sound level in urban sound mixtures. We found that it is a well suited approach to these sound environments because it easily takes into account the overlap between the multiple sound sources present in the cities and it is adapted to monophonic sensor networks. Different versions of NMF have been studied as a supervised and semi-supervised approach. On a large corpus of sounds, the supervised approach proves to be too restrictive to be adapted to different sound environments whereas the semi-supervised approach has, on the contrary, too many degrees of freedom on the mobile dictionary $\mathbf{W_r}$, decreasing its performance especially when the traffic is predominant. The proposed approach, named threshold initialized NMF achieves the lowest average error on the entire corpus. With this method, the $\mathbf{W}$ is initialized with road traffic spectra, updated and the dictionary elements that are similar to the road traffic spectra are then extracted by hard thresholding.
The study of the error according each $TIR$ reveals that this method is not the most efficient on each $TIR$. But it should be remind that one case, $TIR$ = -12 dB, is a rarer case than other values in an urban environment and is intended to test the performance limitations of the NMF TI. If this case was putted aside, TI-NMF reveals to be the only method to reach an $MAE$ error inferior  to 3 dB. The MAE error in Table 2 would decrease to 1.39 while SEM NMF would be 2.15 and the baseline would be 1.77. Consequently, for an urban application where the $TIR$ is an unknown information, TI NMF seems to be the most suited version.
A major advantage of the proposed approach is that it is not designed for a specific source. Extending the proposed approach to other sources is also of interest for future research. The method should thus be tested and validated further on a larger and more diverse dataset with, for instance, motorcycles and other noisy traffic elements which are not considered in this study. Furthermore, even if the experiments described in this paper focused on road traffic level detection, changing the dictionary to contain bird sounds would lead to an estimator of the presence of birds.
