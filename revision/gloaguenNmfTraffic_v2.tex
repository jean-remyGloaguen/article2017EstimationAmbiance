...
The study of the error according each $TIR$ reveals that this method is not the most efficient on each $TIR$, but it has to be remind that, this parameter is not available in practical case. TI-NMF can be considered as the approach that limit the error in the different environment. \ml{a reecrire ici et dans les resultats, les conditions de TIR les plus souvent vue c'est ca, et pour ces modes pris dans leur ensemble, la TI est celle qui marche le mieux.}
...
A major advantage of the proposed approach is that it is not designed for a specific source. Extending the proposed approach to other sources is also of interest for future research. The method should thus be tested and validated further on a larger and more diverse dataset with, for instance, motorcycles and other noisy traffic elements which are not considered in this study. Furthermore, even if the experiments described in this paper focused on road traffic level detection, changing the dictionary to contain bird sounds would lead to an estimator of the presence of birds.
 ...
 %Also, performance improvement could be achieved by the addition of constraints such as sparsity \cite{hoyer2004non} and smoothness \cite{virtanen_monaural_2007} of the low rank matrices.
